...
var options =
    require("nomnom")
        .script("jsonlint")
        .options({
          file : {position : 0, help : "file to parse; otherwise uses stdin"},
          version : {
            flag : true,
            string : '-v, --version',
            help : 'print version and exit',
            callback : function() { return require("../package").version; }
          },
          sort : {
            flag : true,
            string : '-s, --sort-keys',
...
if (options.compact) {
  ...
  parser.parseError = parser.lexer.parseError = function(str, hash) {
    console.error(fileName + 'line ' + hash.loc.first_line + ', col ' +
                  hash.loc.last_column + ', found: \'' + hash.token +
                  '\' - expected: ' + hash.expected.join(', ') + '.');
    throw new Error(str);
  };
}
...
function parse(source) {
  ...
  try {
    parsed =
        options.sort ? sortObject(parser.parse(source)) : parser.parse(source);

    if (options.validate) {
      var env = JSV.createEnvironment(options.env);
      var schema =
          JSON.parse(fs.readFileSync(path.normalize(options.validate), "utf8"));
    ...
    if (options.forcePrettyPrint) {
       ...
       * If we failed to validate, run our manual formatter and then re-validate
       * so that we can get a better line number. On a successful validate, we
       * don't want to run our manual formatter because the automatic one is
...
function main(args) {
  ...
  if (options.file) {
    ...
    stdin.on('data', function(chunk) { source += chunk.toString('utf8'); });
